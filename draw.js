function initializeBoard(e) {
  ...
  drawPad.changeBtnState(freeHand, false);
}
...
class Canvas {
  ...
  changeBtnState(btn, on) {
    const id = btn.id;
    switch (id) {
      case "text":
        !on ? btn.classList.add("selected") : btn.classList.remove("selected");
        break;
      case "freehand":
        !on ? btn.classList.add("selected") : btn.classList.remove("selected");
        break;
  ...
  draw(e) {
    if (this.drawing && !this.createText && !this.drawingLine) {
      const { x, y } = this.getMousePos(e);
      this.ctx.lineTo(x, y);
  ...
  endDrawTextBox(e) {
    ...
    this.createText = false;
    this.changeBtnState({ id: "text" }, true);
  }
  ...
  createTextBox(e) {
    ...
    this.changeBtnState(e.target, false);
  }
